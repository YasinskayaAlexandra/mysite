...
<head>
    <meta charset="UTF-8">
    <title>Шкап - Интернет магазин|Администраторская панель</title>
...
<body>
    ...
    <div class="add">
        <a href="/src/adminformadd.php">
            <button class="butt" type="submit">Добавить товар</button>
        </a>
    </div>
    <div class="product">
        <?
        require "C:\OpenServer\domains\shopshkap\lib\connect.php";
        $queryProduct = "SELECT * FROM `products` ORDER BY `id` DESC";
        ...
        foreach ($products as $product) { ?>
            <div class="item">
                ...
                <form class="td" action="/src/adminformupdate.php" method="post">
                    <input type="hidden" name="id" value="<?= $product["id"]; ?>" />
                    <button class="butt" type="submit">Изменить</button>
                </form>
                <form class="td" action="/lib/delete.php" method="post">
                    <input type="hidden" name="id" value="<?= $product["id"]; ?>" />
                    <button class="butt" type="submit">Удалить</button>
                </form>
            </div>
        <?
        } ?>
